# -*- coding: utf-8 -*-
"""
Created on Fri Sep 24 22:36:05 2021
...
PITCH_PROGRAM_COMPLETE = 150000.0
...
while ASCENT | COASTING | ORBITAL_INSERTION:
    ...
    if ASCENT:
        ...
        # staging
        if BOOSTER_STAGE_1:
            if vessel.resources_in_decouple_stage(stage=6, cumulative=False).amount('LiquidFuel')<0.1:
                staging_sequence(tandemStaging=False)
                BOOSTER_STAGE_1=False
                BOOSTER_STAGE_2=True
        if BOOSTER_STAGE_2:
            if vessel.resources_in_decouple_stage(stage=5, cumulative=False).amount('LiquidFuel')<0.1:
                staging_sequence(tandemStaging=False)
                BOOSTER_STAGE_2=False
                CORE_STAGE=True
        if CORE_STAGE:
